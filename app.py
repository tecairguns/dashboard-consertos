...
sidebar = html.Div(
    [
        ...
        dcc.Input(
            id="filtro-busca", type="text", placeholder="Digite ou clique no gráfico...", debounce=True, 
...
content = html.Div(
    [
        ...
        # KPIs
        dbc.Row([
            dbc.Col(dbc.Card([html.H6("Total Consertos", className="text-muted"), html.H3(id="kpi-total", className="fw-bold", style={"color": COLOR_TEXT_TITLE})], style=CARD_STYLE), width=12, md=3, className="mb-3"),
            dbc.Col(dbc.Card([html.H6("Tempo Médio (Dias)", className="text-muted"), html.H3(id="kpi-media", style={"color": COLOR_TEXT_TITLE, "fontWeight": "bold"})], style=CARD_STYLE), width=12, md=3, className="mb-3"),
            dbc.Col(dbc.Card([html.H6("Modelo Crítico", className="text-muted"), html.H4(id="kpi-modelo", style={"color": COLOR_TEXT_TITLE, "fontWeight": "bold", "fontSize": "1.2rem"})], style=CARD_STYLE), width=12, md=3, className="mb-3"),
            dbc.Col(dbc.Card([html.H6("Taxa Reincidência", className="text-muted"), html.H3(id="kpi-reincidencia", style={"color": COLOR_TEXT_TITLE, "fontWeight": "bold"})], style=CARD_STYLE), width=12, md=3, className="mb-3"),
        ]),
                ...
                html.Div([
                    ...
                    html.Div(
                        dcc.Graph(
                            ...
                            style={"margin": "0"} # Remove margens padrão do componente
                        ),
                        # Estilos do Container que FORÇAM o scroll
                        style={
                            "height": "500px",       # Altura VISÍVEL na tela
                            "overflowY": "scroll",   # Força barra de rolagem vertical
                            "overflowX": "hidden",   # Esconde rolagem horizontal
                            "display": "block",
...
def update_dashboard(busca_modelo, filtro_ano, filtro_mes, filtro_categoria, filtro_garantia, filtro_tipo):
    ...
    fig_main = px.bar(df_chart, x="Mes_nome", y="Quantidade", color="Ano", barmode="group", text_auto=True, template="plotly_white", color_discrete_sequence=COLOR_SEQUENCE)
    fig_main.update_layout(xaxis={"title": ""}, yaxis={"title": "Qtd"}, margin=dict(l=20, r=20, t=30, b=20), font={"color": COLOR_TEXT_TITLE})

    # --- GRÁFICO MODELOS (LÓGICA DO SCROLL) ---
    ...
    # 1. Altura da linha base (barras + espaçamento)
    altura_linha = 35 
    # 2. Altura Total Real do Gráfico (Qtd Itens * Altura Linha)
    # Se tiver poucos itens, mantemos uma altura mínima de 400px para não ficar feio.
    ...
    # 3. APLICAÇÃO CRÍTICA DA ALTURA
    fig_modelos.update_layout(
        height=altura_total,     # <--- ISSO É O QUE FAZ FUNCIONAR. O gráfico SVG terá esse tamanho exato em pixels.
        autosize=True,           # Garante que a largura se ajuste ao container
        yaxis={"title": ""}, 
        xaxis={"title": ""},
    ...
    fig_cat = px.bar(df_cat.sort_values("Quantidade", ascending=True), x="Quantidade", y="Categoria", orientation="h", text="Quantidade", template="plotly_white")
    fig_cat.update_traces(marker_color=COLOR_GRAPH_MAIN, textposition="outside")
    fig_cat.update_layout(yaxis={"title": ""}, xaxis={"title": ""}, margin=dict(l=20, r=20, t=20, b=20), font={"color": COLOR_TEXT_TITLE})

    # G. Tipo
    df_tipo_chart = dff["Tipo"].value_counts().reset_index()
    df_tipo_chart.columns = ["Tipo", "Quantidade"]
    fig_tipo = px.pie(df_tipo_chart, values="Quantidade", names="Tipo", hole=0.6, template="plotly_white", color_discrete_sequence=COLOR_SEQUENCE)
    fig_tipo.update_layout(margin=dict(l=20, r=20, t=20, b=20), showlegend=True, font={"color": COLOR_TEXT_TITLE})
